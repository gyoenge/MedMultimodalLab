"""STaRN eval — UNI-only MLP baseline, LOOCV on IDC_Bench_Xenium_4.

Usage:
    cd /root/workspace/STaRN
    python eval_only_uni.py

Comparison baseline for eval_with_uni.py.
Only UNI ViT-L patch embeddings are used; no tabular radiomics backbone.

Architecture (UNI frozen, only MLPGeneHead trained)
----------------------------------------------------
  uni → pre-extracted ViT-L emb → (B, 1024) → MLPGeneHead → (B, N_GENES)
"""
...
SAVE_DIR = Path("checkpoints/loocv_only_uni")
...
def run_fold(
    ...
    for epoch in range(HEAD_EPOCHS):
        ...
        for batch in train_loader:
            ...
            total_loss += loss.item() * uni_emb.size(0)
            n_seen     += uni_emb.size(0)

        train_mse = total_loss / n_seen
        val_mse, val_pcc, per_gene = evaluate(head, val_loader, device)
...
def main():
    ...
    # step 1 — ensure UNI embeddings exist
    ...
    print(f"Model      : UNI({UNI_DIM}) → MLP({HEAD_HIDDEN_DIM}) → {len(gene_names)}")
    ...
    for fold, val_id in enumerate(sample_list):
        ...
        best_pcc, per_g = run_fold(fold, val_id, train_ids, gene_names, device)
